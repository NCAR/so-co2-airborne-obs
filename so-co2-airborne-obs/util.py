...
class regress(object):
    ...
    def __init__(self, x, y):
        ...
        self.slope = fitted.params[-1]
        self.intercept = fitted.params[0]

        self.beta = fitted.params[::-1]
        self.stderr_beta = fitted.bse[::-1]
        
        self.ci_slope = fitted.conf_int()[1, :]
        self.ci_intercept = fitted.conf_int()[0, :]
        self.stderr_slope = fitted.bse[-1]
        self.r2 = fitted.rsquared        

        self.summary = fitted.summary()
        self.pval = fitted.pvalues[-1]

    def predict(self, xp):
        return self.ols.predict(sm.add_constant(xp))
    
    def invert(self, yp):
        return (yp - self.intercept) / self.slope
    
    def to_dict(self):
        persist_keys = [
            'slope', 'intercept', 
            'stderr_slope', 'r2', 'pval']
        return {k: np.float(v) for k, v in self.__dict__.items() if k in persist_keys}

    @property
    def sse(self):
        """compute coefficient of determination"""
        return np.sum((self.y - self.predict(self.x))**2) / (self.n - 2)

    @property 
    def pval2(self):
        t = self.ols.params / self.ols.bse
        return (2 * (1 - stats.t.cdf(np.abs(t), self.n - 2)))[-1]
...
class linreg_odr(object):
    ...
    def __init__(self, x=None, y=None, xerr=None, yerr=None):
        ...
        if x is None and y is None:
            self.beta = np.ones((2,))*np.nan
            self.stderr_beta = np.ones((2,))*np.nan
            self.cov_beta = np.ones((2, 2))*np.nan
            self.r2 = np.nan
            self.rmse = np.nan
            self.pval = np.nan
            self.sse_y = np.nan
